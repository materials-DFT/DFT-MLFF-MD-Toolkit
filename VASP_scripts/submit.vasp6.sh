...
#SBATCH --job-name=oms6_md_300K_cont
#SBATCH --nodes=1
#SBATCH --ntasks=32
...
#SBATCH --mail-user=nicoleal@sfsu.edu
#SBATCH --requeue
...
BIN="${BIN:-/software/commercial/vasp_sky/vasp_na/vasp.6.5.1_vtst_cpu/bin/vasp_gam}"
WORKDIR="${SLURM_SUBMIT_DIR:-$(pwd)}"
JOBNUM="${SLURM_JOB_ID:-local}"
cd "${WORKDIR}"
...
append_oszicar_delta() {
  ...
  # NOTE: "total" MUST come from this same awk pass (its own NR at END), not a
  # separate wc -l call. wc -l and awk are two independent reads of a file
  # VASP is actively appending to; if the file grows between them, the old
  # code trusted the (now stale) wc -l count as "how far we got," so the next
  # poll re-copied the gap -> duplicate lines/frames in the merged file.
  local total
  total=$(awk -v off="$off" -v start=$((done_lines+1)) -v dst="$dst" '
    NR < start { next }
    /^[[:space:]]*[0-9]+[[:space:]]+T=/ {
      match($0,/^[[:space:]]*/)
...
append_outcar_delta() {
  local src="$1" dst="$2" done_bytes copied
  done_bytes=$(sanitize_uint "${3:-0}")
  [[ -f "$src" ]] || { echo "$done_bytes"; return; }
  # Same race as append_oszicar_delta, byte-flavored: "copied" is measured on
  # the exact stream that was just appended to dst, not via a separate wc -c
  # taken before/after dd. iflag=skip_bytes+bs=1M also avoids the byte-at-a-
  # time dd (bs=1) that made the old race window wider on multi-GB OUTCARs.
  copied=$(dd if="$src" bs=1M skip="$done_bytes" iflag=skip_bytes status=none 2>/dev/null | tee -a "$dst" | wc -c)
  echo "$((done_bytes + copied))"
}

append_xdatcar_delta() {
  local src="$1" dst="$2" off="$3" done_lines="$4" started="$5"
  [[ -f "$src" ]] || { echo "$done_lines $started"; return; }
  # Same fix as append_oszicar_delta: total+started come from this awk pass's
  # own NR/state, never a separate wc -l of the concurrently-growing source.
  read -r total started < <(awk -v off="$off" -v start=$((done_lines+1)) -v started="$started" -v dst="$dst" '
    NR < start { next }
    {
      if (started==0) {
...
clean_shm_on_alloc_nodes() {
  ...
  scontrol show hostnames "${SLURM_JOB_NODELIST}" | sort -u | while read -r n; do
    [[ -n "$n" ]] || continue
...
echo "=== live-merge vasp_gam SLURM ==="
echo "Start: $(date -Is)"
...
printf '%s %s\n' "${JOBNUM}" "${DONE_BEFORE}" > "${WORKDIR}/.cj_baseline"
clean_shm_on_alloc_nodes
