...
class MDDirectoryProcessor:
    ...
    def __init__(
        ...
        ensemble: str = "npt",
        code: str = "vasp",
        lammps_timestep: float = 0.001,
        lammps_target_steps: int = 30000,
    ):
    ...
    def write_lammps_input(
        ...
        """Write LAMMPS input file for Allegro/NequIP with NPT or NVT."""
...
timer         timeout 1:59:30 every 100
...
pair_coeff    * * ${{model}} {' '.join(species_symbols)}
...
variable      Tmd equal {temperature:.1f}
'''
...
label do_vel_create
velocity all create ${{Tmd}} 12345 mom yes rot no dist gaussian
label after_vel_create

# {ensemble_label} ensemble: Langevin thermostat
# VASP LANGEVIN_GAMMA = {' '.join(f'{g:.3f}' for g in md_params.langevin_gamma)} ({' '.join(species_symbols)})
# LAMMPS scale = 1/(damp*gamma); damp = {lang_damp} ps
...
fix baro all press/langevin tri ${{Ptarget}} ${{Ptarget}} ${{baro_pd}} temp ${{Tmd}} ${{Tmd}} 48280 flip yes friction ${{friction_L}}
'''
        
        content += '''fix nve all nve

# Trajectory output
dump 1 all extxyz 1 trajectory.extxyz
dump_modify 1 sort id
dump_modify 1 element '''
        content += ' '.join(species_symbols)
        content += '''
...
# Run simulation
'''
        content += f"timestep {self.lammps_timestep}\n"
        content += "run ${target} upto\n\n"
        content += "write_restart restart.lmp.last\n"
...
Examples:
  ...
  python3 prepare_directories_for_md.py /path/to/structures --vasp --temps 300,700
    ...
    parser.add_argument("--lammps-steps", type=int, default=30000, help="Target steps for LAMMPS (default: 30000)")
    parser.add_argument("--lammps-timestep", type=float, default=0.001, help="LAMMPS timestep in ps (default: 0.001)")
