...
1. Directory Preparation:
   ...
   - Always writes a complete electronic block (ENCUT=520, ISPIN=2 with an
     auto-generated MAGMOM, METAGGA=SCAN, ISMEAR=0, LORBIT=10, ...) even when
     no parent INCAR exists, and refuses to write an INCAR that would fall back
     to plain PBE / ENCUT=ENMAX / ISPIN=1
...
2. MD Parameter Optimization:
   ...
   - POTIM fixed at 1.0 fs; ISTART=0, ICHARG=2 (fresh start, atomic charge density)
...
# Initial moments written into MAGMOM, per element symbol (mu_B). Any species
# not listed here gets 0.0.
DEFAULT_MAGMOM = {"Mn": 3.0}

# Floors enforced on every INCAR written; see validate_electronic_params().
MIN_ENCUT = 520.0
REQUIRED_METAGGA = "SCAN"
REQUIRED_ISPIN = "2"
REQUIRED_ISMEAR = "0"

# Fresh start every time: ISTART=0 ignores any stale WAVECAR, ICHARG=2 builds the
# charge density from a superposition of atomic charges. Overridable per run with
# --istart / --icharg.
DEFAULT_ISTART = "0"
DEFAULT_ICHARG = "2"

DEFAULT_PARAMS = {
    "ISTART": DEFAULT_ISTART,
    "ICHARG": DEFAULT_ICHARG,
    "IBRION": "0",
    "POTIM": "1",
    "ISIF": "3",
}
DEFAULT_PARAMS.update(ELECTRONIC_DEFAULTS)
...
class MDDirectoryProcessor:
    ...
    def __init__(
        ...
        electronic_overrides: Optional[Dict[str, str]] = None,
        allow_unsafe_electronic: bool = False,
        istart: str = DEFAULT_ISTART,
        icharg: str = DEFAULT_ICHARG,
    ):
        ...
        self.electronic_overrides = dict(electronic_overrides or {})
        self.allow_unsafe_electronic = allow_unsafe_electronic
        self.istart = istart
        self.icharg = icharg
        self.processed_dirs = []
        self.skipped_dirs = []
    ...
    @staticmethod
    def build_magmom(species_list: List[AtomicSpecies]) -> str:
        """MAGMOM in POSCAR species order, e.g. '1*0.0 8*3.0 16*0.0'."""
        return ' '.join(
            f"{s.count}*{DEFAULT_MAGMOM.get(s.symbol, 0.0):.1f}"
            for s in species_list
        )
    ...
    def validate_electronic_params(self, params: Dict[str, Any], incar_path: str):
        """Refuse to write an INCAR that would run at the wrong level of theory."""
        problems = []

        try:
            encut = float(str(params.get('ENCUT', 0)).strip())
        except (TypeError, ValueError):
            encut = 0.0
        if encut < MIN_ENCUT:
            problems.append(
                f"ENCUT = {params.get('ENCUT', '<missing>')} is below the required "
                f"{MIN_ENCUT:.0f} eV"
            )

        metagga = str(params.get('METAGGA', '')).strip().upper()
        if metagga != REQUIRED_METAGGA:
            problems.append(
                f"METAGGA = {params.get('METAGGA', '<missing>')} -- plain PBE is not "
                f"allowed; expected {REQUIRED_METAGGA}"
            )

        if str(params.get('ISPIN', '')).strip() != REQUIRED_ISPIN:
            problems.append(
                f"ISPIN = {params.get('ISPIN', '<missing>')} -- spin-polarized "
                f"ISPIN = {REQUIRED_ISPIN} is required"
            )
        elif not str(params.get('MAGMOM', '')).strip():
            problems.append("MAGMOM is missing for a spin-polarized run")

        if str(params.get('ISMEAR', '')).strip() != REQUIRED_ISMEAR:
            problems.append(
                f"ISMEAR = {params.get('ISMEAR', '<missing>')} -- Gaussian "
                f"ISMEAR = {REQUIRED_ISMEAR} is required"
            )
        ...
        message = (
            f"Unsafe electronic settings for {incar_path}:\n  - "
            + "\n  - ".join(problems)
        )
        if self.allow_unsafe_electronic:
            self.log(
                f"WARNING: {message}\n  (writing anyway: --allow-unsafe-electronic)",
                "WARNING",
            )
            return
        raise ValueError(
            message
            + "\n\nRefusing to write this INCAR. Fix the source INCAR, pass an "
              "override (--encut / --algo / --metagga / --ispin), or re-run with "
              "--allow-unsafe-electronic if this is genuinely intended."
        )
    ...
    def write_vasp_incar(
        ...
        all_params['ISTART'] = self.istart
        all_params['ICHARG'] = self.icharg
        ...
        # Electronic settings are never left to VASP's defaults. Anything the
        # parent INCAR supplied is kept (so a hand-tuned ALGO survives); anything
        # missing is filled in here, and the result is checked before writing.
        all_params = self.apply_electronic_defaults(all_params, species_list)
        self.validate_electronic_params(all_params, incar_path)
...
"""
    ...
    def process_structure_dir(
        ...
        # Complete and check the electronic block up front, before any directory
        # is created or any file is copied: a refusal must leave nothing behind
        # that could be submitted by mistake.
        if effective_code == "vasp":
            existing_params = self.apply_electronic_defaults(existing_params, species_list)
            self.validate_electronic_params(existing_params, incar_path)
        ...
        for temp in target_temperatures:
            ...
            if not dry_run:
                ...
                if effective_code == "vasp":
                    ...
                    self.write_vasp_incar(
                        temp_incar_path, existing_params, temp, md_params, species_list
                    )
...
Examples:
  ...
  python3 prepare_directories_for_md.py /path/to/structures --vasp --temps 300,700
    ...
    # Electronic structure (always written; these only override the defaults)
    elec = parser.add_argument_group(
        "electronic structure",
        "Defaults: ENCUT=520, ISPIN=2 (+auto MAGMOM), METAGGA=SCAN, ISMEAR=0, "
        "LORBIT=10. Written to every INCAR even with no parent INCAR present."
    )
    elec.add_argument("--encut", type=float, help="Override ENCUT in eV (default 520)")
    elec.add_argument("--algo", help="Override ALGO (default Normal; inherited value wins otherwise)")
    elec.add_argument("--metagga", help="Override METAGGA (default SCAN)")
    elec.add_argument("--ispin", type=int, choices=[1, 2], help="Override ISPIN (default 2)")
    elec.add_argument("--ismear", type=int, help="Override ISMEAR (default 0)")
    elec.add_argument("--magmom", help="Override MAGMOM verbatim (default: auto from POSCAR species)")
    elec.add_argument("--istart", default=DEFAULT_ISTART,
                      help=f"ISTART written to every INCAR (default {DEFAULT_ISTART})")
    elec.add_argument("--icharg", default=DEFAULT_ICHARG,
                      help=f"ICHARG written to every INCAR (default {DEFAULT_ICHARG})")
    elec.add_argument("--allow-unsafe-electronic", action="store_true",
                      help="Downgrade the electronic-settings check to a warning. "
                           "Without this, an INCAR that would run plain PBE, ENCUT<520, "
                           "ISPIN=1 or ISMEAR!=0 is refused.")
    ...
    if args.encut is not None:
        electronic_overrides["ENCUT"] = f"{args.encut:g}"
    if args.algo:
        electronic_overrides["ALGO"] = args.algo
    if args.metagga:
        electronic_overrides["METAGGA"] = args.metagga
    if args.ispin is not None:
        electronic_overrides["ISPIN"] = str(args.ispin)
    if args.ismear is not None:
        electronic_overrides["ISMEAR"] = str(args.ismear)
    if args.magmom:
        electronic_overrides["MAGMOM"] = args.magmom
    ...
    processor = MDDirectoryProcessor(
        ...
        electronic_overrides=electronic_overrides,
        allow_unsafe_electronic=args.allow_unsafe_electronic,
        istart=args.istart,
        icharg=args.icharg,
    )
