#!/usr/bin/env python3
"""
VASP/LAMMPS MD Directory Preparation and Optimization Suite (evaluation-set variant)
=====================================================================================

Restored/verified snapshot of prepare_directories_for_md.py as it stood at
commit 6604f09 (2026-06-29) -- the last commit before the "stable workflow"
rewrite in 062bdab (2026-07-17) added a CG-minimize + temperature-ramp
burn-in stage, a 0.1 fs production timestep, and a 1,000,000-step default
target to the LAMMPS writer.

Why this file exists: the production NVT runs under
palmetto_calculations/test_set/oms6/md/NVT/neutral/1k/2_layers/{300K,700K}
(VASP) and lammps_md/allegro/k_phases/oms6/3/NVT/1k/2_layers/{300K,700K}
(LAMMPS/Allegro) were generated by this version of the logic -- confirmed by
regenerating both outputs from the actual data.lammps/POSCAR inputs and
diffing:
  - LAMMPS in.allegro: byte-for-byte identical when species masses are read
    from data.lammps (read_lammps_data), for both 300K and 700K.
  - VASP LANGEVIN_GAMMA: exact match (300K: 1.682 1.372 2.876 ;
    700K: 2.570 2.095 4.393), plus matching ISIF/MDALGO/POTIM/section layout.
The current (post-062bdab) prepare_directories_for_md.py no longer
reproduces those LAMMPS inputs (different timestep, run length, burn-in
stage, neighbor/dump settings) -- use *this* file when you need output that
matches the already-completed 1k/2_layers NVT evaluation set.

Known caveat: per-run manual tuning on top of this script's output was
observed in the production INCARs (ISTART/ICHARG flipped to 0/2 for a fresh
start, and NELMIN/NELM/ALGO hand-adjusted per temperature for electronic
convergence) -- those fields are simply carried through from whatever INCAR
existed before this script ran (see write_vasp_incar's `electronic_keys`
pass-through), so they are expected to need the same kind of manual review
before a production VASP run, not something this script decides for you.

------------------------------------------------------------------------
Original docstring (unchanged below this line)
------------------------------------------------------------------------
...
Usage:
    python3 prepare_for_md_evaluation.py <directory_path> [options]

Examples:
    # VASP NPT (default)
    python3 prepare_for_md_evaluation.py /path/to/structures --temps 300,700

    # VASP NVT
    python3 prepare_for_md_evaluation.py /path/to/structures --nvt --temps 300,700

    # LAMMPS NPT (temperature from directory name when present, e.g. 700K/)
    python3 prepare_for_md_evaluation.py /path/to/NPT --lammps

    # LAMMPS NVT
    python3 prepare_for_md_evaluation.py /path/to/structures --lammps --nvt --temps 300,700

Author: AI Assistant
Version: 2.0-eval (Combined VASP/LAMMPS, NPT/NVT; frozen at commit 6604f09)
"""
...
# Auto-generated by prepare_for_md_evaluation.py
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Comprehensive VASP/LAMMPS MD directory preparation and optimization (evaluation-set variant, frozen at commit 6604f09)",
...
Examples:
  ...
  # Force LAMMPS mode
  python3 prepare_for_md_evaluation.py /path/to/structures --lammps --temps 700

  # Force VASP mode
  python3 prepare_for_md_evaluation.py /path/to/structures --vasp --temps 300,700
        """
    )
